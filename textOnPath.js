...
export default class TextOnPath {
  constructor(svgEl) {
    ...
    const filterId =
      this.DOM.text.getAttribute("filter") &&
      this.DOM.text
        .getAttribute("filter")
        .match(/url\(["']?([^"']*)["']?\)/)[1];
    ...
    this.filterPrimitive =
      filterType && filterId && new FilterPrimitive(filterType, filterId);
...
class FilterPrimitive {
  ...
  update(distance) {
    const types = {
        ...
        this.DOM.el.setAttribute(
          ...
          clamp(
            map(
              ...
              0,
              400,
              this.DOM.el.dataset.minDeviation || 0,
              this.DOM.el.dataset.maxDeviation || 10
            ),
            this.DOM.el.dataset.minDeviation || 0,
            this.DOM.el.dataset.maxDeviation || 10
          )
      ...
      distortion: () =>
        (this.DOM.el.scale.baseVal = clamp(
          map(
            distance,
            0,
            200,
            this.DOM.el.dataset.minScale || 0,
            this.DOM.el.dataset.maxScale || 100
          ),
          this.DOM.el.dataset.minScale || 0,
          this.DOM.el.dataset.maxScale || 100
        ))
